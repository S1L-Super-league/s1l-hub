...
(function(){
  ...
  var PRIO={de:['de','en','fr','it','es','pt','tr','ru'],en:['en','de','fr','it','es','pt','tr','ru'],fr:['fr','en','de','it','es','pt','tr','ru'],it:['it','en','de','fr','es','pt','tr','ru'],es:['es','pt','en','de','fr','it','tr','ru'],pt:['pt','es','en','de','fr','it','tr','ru'],tr:['tr','en','de','fr','it','es','pt','ru'],ru:['ru','en','de','fr','it','es','pt','tr']};
  ...
  function hasContent(doc){ return !!(doc && (doc.t_orig||doc.t_de||doc.t_en||doc.t_fr||doc.t_it||doc.t_es||doc.t_pt||doc.t_tr||doc.t_ru||doc.img||doc.type)); }
  /* Übersetzungen der R4-Bedien-Elemente (Buttons, Editor, Tooltips) */
  var UI={
    add:{de:'➕ Info-Kachel (immer offen)',en:'➕ Info tile (always open)',fr:"➕ Tuile d'info (toujours ouverte)",it:'➕ Riquadro info (sempre aperto)',es:'➕ Tarjeta de info (siempre abierta)',pt:'➕ Cartão de info (sempre aberto)',tr:'➕ Bilgi kartı (her zaman açık)',ru:'➕ Инфо-плитка (всегда открыта)'},
    addcollap:{de:'➕ aufklappbare Kachel (mit Überschrift)',en:'➕ Collapsible tile (with heading)',fr:'➕ Tuile déroulante (avec titre)',it:'➕ Riquadro espandibile (con titolo)',es:'➕ Tarjeta desplegable (con título)',pt:'➕ Cartão expansível (com título)',tr:'➕ Açılır kart (başlıklı)',ru:'➕ Раскрывающаяся плитка (с заголовком)'},
    addpage:{de:'➕ Seite (Kachel → Seite)',en:'➕ Page (tile → page)',fr:'➕ Page (tuile → page)',it:'➕ Pagina (riquadro → pagina)',es:'➕ Página (tarjeta → página)',pt:'➕ Página (cartão → página)',tr:'➕ Sayfa (kart → sayfa)',ru:'➕ Страница (плитка → страница)'},
    up:{de:'hoch schieben',en:'move up',fr:'monter',it:'sposta su',es:'subir',pt:'mover para cima',tr:'yukarı taşı',ru:'вверх'},
    down:{de:'runter schieben',en:'move down',fr:'descendre',it:'sposta giù',es:'bajar',pt:'mover para baixo',tr:'aşağı taşı',ru:'вниз'},
    edit:{de:'ändern',en:'edit',fr:'modifier',it:'modifica',es:'editar',pt:'editar',tr:'düzenle',ru:'изменить'},
    del:{de:'entfernen',en:'remove',fr:'supprimer',it:'rimuovi',es:'eliminar',pt:'remover',tr:'kaldır',ru:'удалить'},
    save:{de:'Speichern',en:'Save',fr:'Enregistrer',it:'Salva',es:'Guardar',pt:'Guardar',tr:'Kaydet',ru:'Сохранить'},
    cancel:{de:'Abbrechen',en:'Cancel',fr:'Annuler',it:'Annulla',es:'Cancelar',pt:'Cancelar',tr:'İptal',ru:'Отмена'},
    translating:{de:'übersetze…',en:'translating…',fr:'traduction…',it:'traduzione…',es:'traduciendo…',pt:'a traduzir…',tr:'çevriliyor…',ru:'перевод…'},
    shrinking:{de:'Bild wird verkleinert…',en:'shrinking image…',fr:"réduction de l'image…",it:'riduzione immagine…',es:'reduciendo imagen…',pt:'a reduzir imagem…',tr:'görsel küçültülüyor…',ru:'сжатие изображения…'},
    needtext:{de:'Bitte Text eingeben.',en:'Please enter text.',fr:'Veuillez saisir un texte.',it:'Inserisci un testo.',es:'Introduce un texto.',pt:'Introduz um texto.',tr:'Lütfen metin girin.',ru:'Введите текст.'},
    toobig:{de:'Bild zu groß — bitte kleineres wählen.',en:'Image too large — pick a smaller one.',fr:'Image trop grande — choisissez-en une plus petite.',it:'Immagine troppo grande — scegline una più piccola.',es:'Imagen demasiado grande — elige una más pequeña.',pt:'Imagem demasiado grande — escolhe uma mais pequena.',tr:'Görsel çok büyük — daha küçük seç.',ru:'Изображение слишком большое — выберите меньше.'},
    saveerr:{de:'Fehler beim Speichern.',en:'Error while saving.',fr:"Erreur lors de l'enregistrement.",it:'Errore durante il salvataggio.',es:'Error al guardar.',pt:'Erro ao guardar.',tr:'Kaydederken hata.',ru:'Ошибка при сохранении.'},
    warn:{de:'⚠️ Achtung: ändert auch die Strategie.',en:'⚠️ Note: this also changes the strategy.',fr:'⚠️ Attention : modifie aussi la stratégie.',it:'⚠️ Attenzione: modifica anche la strategia.',es:'⚠️ Atención: también cambia la estrategia.',pt:'⚠️ Atenção: também altera a estratégia.',tr:'⚠️ Dikkat: stratejiyi de değiştirir.',ru:'⚠️ Внимание: меняет и стратегию.'},
    notePage:{de:'Name der neuen Seite — wird in alle Sprachen übersetzt. Die Kachel wird anklickbar und führt auf die neue Seite.',en:'Name of the new page — translated into all languages. The tile becomes clickable and opens the page.',fr:'Nom de la nouvelle page — traduit dans toutes les langues. La tuile devient cliquable et ouvre la page.',it:'Nome della nuova pagina — tradotto in tutte le lingue. Il riquadro diventa cliccabile e apre la pagina.',es:'Nombre de la nueva página — se traduce a todos los idiomas. La tarjeta se vuelve clicable y abre la página.',pt:'Nome da nova página — traduzido para todos os idiomas. O cartão torna-se clicável e abre a página.',tr:'Yeni sayfanın adı — tüm dillere çevrilir. Kart tıklanabilir olur ve sayfayı açar.',ru:'Название новой страницы — переводится на все языки. Плитка станет кликабельной и откроет страницу.'},
    noteCollap:{de:'Erste Zeile = Überschrift (zum Aufklappen), danach der Text. Wird in alle Sprachen übersetzt.',en:'First line = heading (to expand), then the text. Translated into all languages.',fr:'Première ligne = titre (pour déplier), puis le texte. Traduit dans toutes les langues.',it:'Prima riga = titolo (per espandere), poi il testo. Tradotto in tutte le lingue.',es:'Primera línea = título (para desplegar), luego el texto. Se traduce a todos los idiomas.',pt:'Primeira linha = título (para expandir), depois o texto. Traduzido para todos os idiomas.',tr:'İlk satır = başlık (açmak için), sonra metin. Tüm dillere çevrilir.',ru:'Первая строка = заголовок (для раскрытия), затем текст. Переводится на все языки.'},
    noteText:{de:'Schreib in deiner Sprache — wird automatisch in DE/EN/TR/RU übersetzt.',en:'Write in your language — auto-translated into DE/EN/TR/RU.',fr:'Écris dans ta langue — traduit automatiquement en DE/EN/TR/RU.',it:'Scrivi nella tua lingua — tradotto automaticamente in DE/EN/TR/RU.',es:'Escribe en tu idioma — se traduce automáticamente a DE/EN/TR/RU.',pt:'Escreve no teu idioma — traduzido automaticamente para DE/EN/TR/RU.',tr:'Kendi dilinde yaz — otomatik DE/EN/TR/RU çevrilir.',ru:'Пиши на своём языке — авто-перевод на DE/EN/TR/RU.'},
    img:{de:'📎 Bild:',en:'📎 Image:',fr:'📎 Image :',it:'📎 Immagine:',es:'📎 Imagen:',pt:'📎 Imagem:',tr:'📎 Görsel:',ru:'📎 Изображение:'},
    imgnote:{de:'Nur Spiel-Bezug (Screenshots/Grafiken) — keine privaten Fotos.',en:'Game-related only (screenshots/graphics) — no private photos.',fr:'En rapport avec le jeu uniquement (captures/graphismes) — pas de photos privées.',it:'Solo attinente al gioco (screenshot/grafica) — niente foto private.',es:'Solo relacionado con el juego (capturas/gráficos) — sin fotos privadas.',pt:'Apenas relacionado com o jogo (capturas/gráficos) — sem fotos privadas.',tr:'Sadece oyunla ilgili (ekran/grafik) — özel fotoğraf yok.',ru:'Только по игре (скриншоты/графика) — без личных фото.'},
    imgdel:{de:'Bild entfernen',en:'Remove image',fr:"Supprimer l'image",it:'Rimuovi immagine',es:'Eliminar imagen',pt:'Remover imagem',tr:'Görseli kaldır',ru:'Убрать изображение'},
    phPage:{de:'Name der Seite, z. B. Turbo-Guide',en:'Page name, e.g. Turbo guide',fr:'Nom de la page, p. ex. Turbo guide',it:'Nome della pagina, es. Turbo guida',es:'Nombre de la página, p. ej. Turbo guía',pt:'Nome da página, p. ex. Guia Turbo',tr:'Sayfa adı, örn. Turbo rehberi',ru:'Название страницы, напр. Turbo-гайд'},
    phCollap:{de:'Überschrift (1. Zeile), danach der Text',en:'Heading (line 1), then the text',fr:'Titre (ligne 1), puis le texte',it:'Titolo (riga 1), poi il testo',es:'Título (línea 1), luego el texto',pt:'Título (linha 1), depois o texto',tr:'Başlık (1. satır), sonra metin',ru:'Заголовок (1-я строка), затем текст'},
    fH1:{de:'Überschrift 1',en:'Heading 1',fr:'Titre 1',it:'Titolo 1',es:'Título 1',pt:'Título 1',tr:'Başlık 1',ru:'Заголовок 1'},
    fH2:{de:'Überschrift 2',en:'Heading 2',fr:'Titre 2',it:'Titolo 2',es:'Título 2',pt:'Título 2',tr:'Başlık 2',ru:'Заголовок 2'},
    fBold:{de:'Fett',en:'Bold',fr:'Gras',it:'Grassetto',es:'Negrita',pt:'Negrito',tr:'Kalın',ru:'Жирный'},
    fList:{de:'Aufzählung',en:'List',fr:'Liste',it:'Elenco',es:'Lista',pt:'Lista',tr:'List',ru:'Список'},
    confirmDel:{de:'Diese Kachel entfernen? (rückholbar über die Versionierung)',en:'Remove this tile? (recoverable via versioning)',fr:'Supprimer cette tuile ? (récupérable via le versionnage)',it:'Rimuovere questo riquadro? (recuperabile tramite il versionamento)',es:'¿Eliminar esta tarjeta? (recuperable mediante el versionado)',pt:'Remover este cartão? (recuperável através do versionamento)',tr:'Bu kart kaldırılsın mı? (sürümleme ile geri alınabilir)',ru:'Убрать плитку? (можно вернуть через версии)'},
    newpage:{de:'(neue Seite)',en:'(new page)',fr:'(nouvelle page)',it:'(nuova pagina)',es:'(nueva página)',pt:'(nova página)',tr:'(yeni sayfa)',ru:'(новая страница)'},
    heading:{de:'(Überschrift)',en:'(heading)',fr:'(titre)',it:'(titolo)',es:'(título)',pt:'(título)',tr:'(başlık)',ru:'(заголовок)'},
    hide:{de:'ausblenden (R3 sieht es nicht)',en:'hide (members don\'t see it)',fr:'masquer (les membres ne le voient pas)',it:'nascondi (i membri non lo vedono)',es:'ocultar (los miembros no lo ven)',pt:'ocultar (os membros não veem)',tr:'gizle (üyeler görmez)',ru:'скрыть (участники не видят)'},
    show:{de:'wieder einblenden',en:'show again',fr:'réafficher',it:'mostra di nuovo',es:'mostrar de nuevo',pt:'mostrar de novo',tr:'tekrar göster',ru:'показать снова'},
    hiddenTag:{de:'ausgeblendet',en:'hidden',fr:'masqué',it:'nascosto',es:'oculto',pt:'oculto',tr:'gizli',ru:'скрыто'},
    previewOn:{de:'R3-Vorschau',en:'R3 preview',fr:'Aperçu R3',it:'Anteprima R3',es:'Vista R3',pt:'Pré-visualização R3',tr:'R3 önizleme',ru:'Просмотр R3'},
    previewOff:{de:'Vorschau beenden',en:'exit preview',fr:'Quitter l\'aperçu',it:'Esci anteprima',es:'Salir de la vista',pt:'Sair da pré-visualização',tr:'Önizlemeyi kapat',ru:'Выйти из просмотра'},
    addsection:{de:'➕ Sektionsüberschrift',en:'➕ Section heading',fr:'➕ Titre de section',it:'➕ Titolo di sezione',es:'➕ Título de sección',pt:'➕ Título de secção',tr:'➕ Bölüm başlığı',ru:'➕ Заголовок раздела'},
    phSection:{de:'Überschrift des Abschnitts, z. B. 📌 Das Wichtigste',en:'Section heading, e.g. 📌 The Essentials',fr:'Titre de section, p. ex. 📌 L\'essentiel',it:'Titolo di sezione, es. 📌 L\'essenziale',es:'Título de sección, p. ej. 📌 Lo esencial',pt:'Título da secção, p. ex. 📌 O essencial',tr:'Bölüm başlığı, örn. 📌 En önemlisi',ru:'Заголовок раздела, напр. 📌 Самое важное'},
    noteSection:{de:'Kurze Abschnitts-Überschrift (eine Zeile, Emoji erlaubt) — wird in alle Sprachen übersetzt.',en:'Short section heading (one line, emoji ok) — translated into all languages.',fr:'Titre de section court (une ligne, emoji ok) — traduit dans toutes les langues.',it:'Titolo di sezione breve (una riga, emoji ok) — tradotto in tutte le lingue.',es:'Título de sección corto (una línea, emoji ok) — se traduce a todos los idiomas.',pt:'Título de secção curto (uma linha, emoji ok) — traduzido para todos os idiomas.',tr:'Kısa bölüm başlığı (tek satır, emoji olur) — tüm dillere çevrilir.',ru:'Короткий заголовок раздела (одна строка, эмодзи можно) — переводится на все языки.'},
    secHeading:{de:'(Abschnitt)',en:'(section)',fr:'(section)',it:'(sezione)',es:'(sección)',pt:'(secção)',tr:'(bölüm)',ru:'(раздел)'},
    color:{de:'🎨 Farbe:',en:'🎨 Color:',fr:'🎨 Couleur :',it:'🎨 Colore:',es:'🎨 Color:',pt:'🎨 Cor:',tr:'🎨 Renk:',ru:'🎨 Цвет:'},
    colDefault:{de:'Standard',en:'Default',fr:'Standard',it:'Predefinito',es:'Predeterminado',pt:'Padrão',tr:'Varsayılan',ru:'По умолчанию'},
    publicTag:{de:'🌐 Neu hier?',en:'🌐 New here?',fr:'🌐 Nouveau ?',it:'🌐 Nuovo?',es:'🌐 ¿Nuevo?',pt:'🌐 Novo aqui?',tr:'🌐 Yeni?',ru:'🌐 Впервые?'},
    publicTagTip:{de:'Dieser Inhalt erscheint auch auf der öffentlichen Seite „Neu hier?".',en:'This content also appears on the public "New here?" page.',fr:'Ce contenu apparaît aussi sur la page publique « Nouveau ? ».',it:'Questo contenuto appare anche sulla pagina pubblica «Nuovo?».',es:'Este contenido también aparece en la página pública «¿Nuevo?».',pt:'Este conteúdo também aparece na página pública «Novo aqui?».',tr:'Bu içerik herkese açık "Yeni?" sayfasında da görünür.',ru:'Этот контент также показывается на публичной странице «Впервые?».'}
  };
  ...
  function extractCurrentText(el){
    ...
    clone.querySelectorAll('[class*="lng-"]').forEach(function(n){ var m=n.className.match(/lng-(de|en|fr|it|es|pt|tr|ru)/); if(m && m[1]!==cur) n.remove(); });
    ...
    box.querySelector('.c-save').addEventListener('click', function(){
      ...
      translate(text, function(tr){
        ...
        var nd={ cid:cid, t_orig:text, orig_lang:curLang(), editor:editorName(), tms:Date.now(),
                 t_de:(tr&&tr.de)||'', t_en:(tr&&tr.en)||'', t_fr:(tr&&tr.fr)||'', t_it:(tr&&tr.it)||'', t_es:(tr&&tr.es)||'', t_pt:(tr&&tr.pt)||'', t_tr:(tr&&tr.tr)||'', t_ru:(tr&&tr.ru)||'',
        ...
        if(!(tr&&(tr.de||tr.en||tr.fr||tr.it||tr.es||tr.pt||tr.tr||tr.ru))) nd['t_'+curLang()]=text;  // Worker aus -> Original übernehmen
